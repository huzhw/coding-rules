...
- **memory 文件名用中文且必须带日期后缀 `-YYYY-MM-DD`**，例如 `服务器102-服务清单-2026-08-14.md`、`达梦数据库-MCP-2026-08-14.md`；不用纯英文命名，也不许不带日期的裸文件名
- **每次更新 memory 文件时同步改日期**：改内容的当天把文件名里的日期更新为当日，并同步更新 `MEMORY.md` 索引里该行的链接与「更新于」日期
- `MEMORY.md` 作为索引，一行一条链接，不存正文
- **AI 自动记录一律写 `<项目>/memory/ai/` 子目录**；项目根 `memory/` 只放用户主动记录
- **用户区（memory/ 根）文件 AI 只能读、不能改/新增**，除非用户明确要求更新
- **晋升机制**：AI 记录若值得长期保留，由用户手动移到根目录
- **AI 自动记录（memory/ai/）不提交 git**，项目 `.gitignore` 需排除 `memory/ai/`
- **`memory/ai/` 有独立 `MEMORY.md` 索引**，根 `MEMORY.md` 只管用户记录
